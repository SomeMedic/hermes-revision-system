...
from .diff import DiffCalculator, FileDiff
...
class VCSRepository:
    ...
    def commit(self, message: str, author: str) -> str:
        ...
        self.hook_manager.run_hook(HookType.POST_COMMIT, {
            ...
            'VCS_COMMIT_MESSAGE': message,
            'VCS_COMMIT_AUTHOR': author
        })
    ...
    def get_file_content_from_commit(self, commit_hash: str, file_path: str) -> Optional[str]:
        ...
        return self.get_file_content(file_hash)
    ...
    def diff_commits(self, old_commit: str, new_commit: str) -> List[FileDiff]:
        ...
        for file_path in sorted(all_files):
            ...
            old_content = self.get_file_content(old_hash) if old_hash else ""
            new_content = self.get_file_content(new_hash) if new_hash else ""
    ...
    def revert_commit(self, commit_hash: str) -> str:
        ...
        for diff in diffs:
            if diff.is_new:
                ...
                old_content = self.get_file_content(diff.old_hash)
    ...
    def reset_files(self, files: List[str]) -> None:
        ...
        for file_path in files:
            ...
            content = self.get_file_content(file_hash)
